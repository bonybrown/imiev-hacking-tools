...
class Kwp2000(J2534Channel channel, CanAddress txCanId, CanAddress rxCanId)
{
    ...
    public Kwp2000Result Receive(int timeoutMs = -1, int minimumBytes = 1)
    {
        ...
        while (resultBytes.Count < minimumBytes)
        {
            ...
            if(Debug) Console.WriteLine($"ReadMessages result: {readResult}");
            ...
            {
                if (Debug) Console.WriteLine($"Received message: {BitConverter.ToString(msg.Data)}");
                var d = msg.Data;
                if (d.Length < 5)
                    continue;
...
if(Debug) Console.WriteLine($"Sending service 0x{serviceId:X2} with data [{BitConverter.ToString(data.ToArray())}], timeout {timeoutMs} ms, expectResponse={expectResponse}");
        var sendResult = Send(serviceId, data);
if(Debug) Console.WriteLine($"Send result: {sendResult}");
        if (!sendResult.Success)
            return sendResult;

        if (!expectResponse)
            return Kwp2000Result.Positive([]);
if(Debug) Console.WriteLine($"Waiting for response with timeout {timeoutMs} ms and minimum bytes {minimumResponseBytes}...");
        var rxResult = Receive(timeoutMs, minimumResponseBytes);
if(Debug) Console.WriteLine($"Receive result: {rxResult}");
    ...
    {
        return SendCommand(0x27, [accessLevel], timeoutMs, expectResponse: true, minimumResponseBytes: 4);
    }
